...
class Auditlog_List_Table extends \WP_List_Table {
    ...
    public function get_table_classes() {
        return [ 'widefat', 'fixed', 'striped', 'audit-log-list-table', $this->_args['plural'] ];
    }
        ...
        ?><div class="alignleft actions">
            ...
            <select name="filter_module" id="filter_module">
                ...
                <?php foreach ( $modules as $module ) : ?>
                    <option value="<?php echo esc_attr( $module['component'] ); ?>" <?php selected( $selected_module, $module['component'] ); ?>><?php echo esc_html( $module['component'] ); ?></option>
                <?php endforeach; ?>
            </select>
     ...
     * @return array
     */
    public function erp_log_get_filters() {
        $filters = [
            'today'        => __( 'Today', 'erp' ),
     ...
     * @return string
     */
    public function column_default( $audit_log, $column_name ) {
        switch ( $column_name ) {
            case 'name':
     ...
     */
    public function erp_log_get_start_end_date( $time = '' ) {
        $duration = [];
